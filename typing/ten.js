...
window.addEventListener("load", function () {
  ...
  document.getElementById("atext").innerHTML = x.substring(0, 1750);
  document.getElementById("ctext").focus();
  document.getElementById("ctext").style.height = w3_getStyleValue(
...
function setBlink(a) {
  ...
  document.getElementById("atext").innerHTML =
    document.getElementById("atext").innerText.substr(0, n) +
    "<span style='" +
    blinkstyle +
    "'>" +
    document.getElementById("atext").innerText.substr(n, 1) +
    "</span>" +
    document.getElementById("atext").innerText.substr(n + 1);
...
function ku(e) {
  ...
  for (i = 0; i < c.length; i++) {
    if (
      a[i] == c[i] ||
      (a[i].charCodeAt(0) == 32 && c[i].charCodeAt(0) == 160)
    ) {
      charcount++;
  ...
  document.getElementById("words").innerHTML = Math.ceil(charcount / 5 / 10);
}
...
// Start the timer and display formatted time
function initCount() {
  document.getElementById("timefooter").innerHTML = "";
  updateTimeDisplay(timelength);
